...
class ROVControlPanel(QGraphicsView):
    thrust_updated = pyqtSignal(list)
    ...
    def __init__(self):
        ...
        self.thruster_angles_deg = {
            1: -45,   2: 45+180,    3: 45,   4: -45+180,     # Horizontals
    ...
    def update_controls(self):
        ...
        # --- MAP FLOATS (-1.0 to 1.0) TO INTEGERS (0 to 255) ---
        # Neutral is 127, Min is 0, Max is 255
        scaled_outputs = [int(round((val + 1.0) * 127.5)) for val in outputs]
        # Ensure strict bounds clamping
        scaled_outputs = [max(0, min(255, val)) for val in scaled_outputs]

        # Emit the scaled list of integers to the controller/rpi
        self.thrust_updated.emit(scaled_outputs)
